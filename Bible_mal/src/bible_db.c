...
_load_appdata(appdata_s *ad)
{
	...
	if (!existing)
	{
		preference_set_int("fontsize", 23);
		edje_text_class_set("GTA1", "Tizen:style=Regular", 23);
		edje_text_class_set("GTA1B", "Tizen:style=Bold", 23);
		edje_text_class_set("GTA1L", "Tizen:style=Light", 23);
	}
	else
	{
		int val = 25;
...
_get_verse_count_query(void *data, int book, int chapter)
{
	...
	sprintf(query, "select count(Versecount) from bible where Book='%s' and Chapter=%d;", Books[book], chapter);
...
_get_chapter_count_query(void *data, int book)
{
	...
	sprintf(query, "select count(distinct Chapter) from bible where Book='%s';", Books[book]);
...
_query_chapter(void *data, int book, int chapter)
{
    ...
    sprintf(query, "select m_verse from bible where Book='%s' and Chapter=%d", Books[book], chapter);
